...
mod tests {
    ...
    async fn test_login_url_explicit_redirect_with_permissions() {
        ...
        let f =
            |backend, user, state| async move { verify_permissions(backend, user, &TestState { req_perm: vec![] })
                .await };

        let re = RequireBuilder::<TestBackend>::new().fallback(
            RedirectFallback::new()
                .redirect_field("next_url")
                .login_url("/login?next_url=%2Fdashboard"),
        );
